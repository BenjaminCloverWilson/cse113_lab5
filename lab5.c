 ...
 * Assignment: Lab 5
 *
 * @brief a program that tests the functions in array.c utilizing structures
 *
 * @details
 *
 * @bug none
...
/* Structure that holds summary values. Also, initializes main array structure */
struct summary_s {
    size_t size;
    int max;
    int min;
...
void print_summary(struct summary_s summary);

struct summary_s get_summary(int a[], struct summary_s summary);
...
int main(void)
{
    ...
    */
    ...
    /* Custom test array structures */
    /*
    struct summary_s summary_zero;
    struct summary_s summary_one;
    struct summary_s summary_up;
    struct summary_s summary_down;
    struct summary_s summary_jostle;
    */
    ...
    /* Calculates the size of the array to store in respective structure*/
    summary.size = sizeof(a) / sizeof(int);

    /* Calculates the size of the test arrays to store in respective structures*/
    /*
    summary_zero.size = sizeof(zero) / sizeof(int);
    summary_one.size = sizeof(one) / sizeof(int);
    summary_up.size = sizeof(up) / sizeof(int);
    summary_down.size = sizeof(down) / sizeof(int);
    summary_jostle.size = sizeof(jostle) / sizeof(int);
    */

    /* Prints the array and gets its information */
    summary = get_summary(a, summary);

    /* Prints the test arrays and gets their information to store in declared structure*/
    /*
    summary_zero = get_summary(zero, summary_zero);
    summary_one = get_summary(one, summary_one);
    summary_up = get_summary(up, summary_up);
    summary_down = get_summary(down, summary_down);
    summary_jostle = get_summary(jostle, summary_jostle);
    */

    /* Prints out the summary of the information ascertained from the main array */
    print_summary(summary);

    /* Print information about the test arrays */
    /*
    print_summary(summary_zero);
    print_summary(summary_one);
    print_summary(summary_up);
    print_summary(summary_down);
    print_summary(summary_jostle);
    */
...
/** Calls all the functions in array.c to get statistical info about the array
 * while also printing the array at multiple points to see what is happening.
 * @param a[] the array where numbers used in the functions are taken from.
 * @param summary The structure containing all 13 of the variables that
 * store an array's statistical info calculated by the called functions..
 */
struct summary_s get_summary(int a[], struct summary_s input)
{

    /* Keeps track of output and size of output is the same as the input */
    struct summary_s output;
    output.size = input.size;

    /** prints out the array before any
     * modifications are made to the array by the functions
     */
    printf("Unedited array:\n");
    print_array(a, input.size);

    /* Function calls */
    output.max = find_max(a, input.size);
    output.min = find_min(a, input.size);
    output.mid = midpoint(output.min, output.max);
    output.LTM = get_count(a, input.size, LT, output.mid);
    output.GTM = get_count(a, input.size, GT, output.mid);
    output.lin = linear_search(a, input.size, output.mid);
    output.sum = sum_ele(a, input.size);
    output.average = avg_ele(input.size, output.sum);
    output.even = count_even(a, input.size);
    output.odd = count_odd(a, input.size);
    output.div = divisible_count(a, input.size, output.min);

    /** Median is last to ensure that all other functions and calls work
    * without the sort() call within median_ele()
    */
    output.med = median_ele(a, input.size);

    /* Prints the now sorted array */
    printf("\nSorted array:\n");
    print_array(a, input.size);

    /* Reverse array and prints it out */
    reverse(a, input.size);

    /* Returns all members in the structure */
    return output;
}